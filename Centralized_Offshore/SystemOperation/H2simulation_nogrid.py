...
def simplified_sim(df_NorthSea, EL_P_nom, components, energy, EL, kWh_factor, L_pipe, EX_length, IA_length, eta_rect, bound_tech):
    ...
    T_EXT = 15 #[15°C]
    P_wind = df_NorthSea['Wind Power [kW]']
    'power fluxes'
    ...
    EL_tr_losses = (IA_length * components['IA_cables']['losses']) #[kW]
    ...
    for i in range(len(P_RES)):
        ...
        EL_P_given = min(P_requested, (P_RES.iloc[i] * (1 - EL_tr_losses) * eta_rect)) #[kW]
        ...
        P_aux.append((W_RO + W_cool) / kWh_factor)  #[kW]
        P_compr.append(W_compr* P_expl)             #[kW]
        P_grid.append(W_compr* P_expl)              #[kW]
    ...
    E_aux_y = (sum(P_aux) / kWh_factor / annuality_factor) / 1000                                                     #[MWh/y]  yearly grid contribution to cooling and desalination production
