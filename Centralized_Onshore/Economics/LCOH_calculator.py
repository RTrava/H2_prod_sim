...
def LCOH_func(sizes, components, EL, energy, r, lifetime, SF, cables, bound, rep):
    ...
    if cables == 'DC_cables': substr= 'DC_electric_substructure'
    else:  substr= 'AC_electric_substructure'
    
    'component sizes'
    components[EL]['size']                      =   sizes['EL']         # MW
    components['WT']['size']                    =   sizes['WF']         # MWp
    components['Compressor_on']['size']         =   sizes['Compressor'] # MWp
    components['RO_unit']['size']               =   sizes['RO_unit']    # m3/d
    components['IA_cables']['size']             =   sizes['IA_Cables']  # km
    components[cables]['size']                  =   sizes['EX_Cables']  # km
    components[substr]['size']                  =   sizes['WF']         # km
    components['H2_onshore']['size']            =   sizes['EL']         # MW
    ...
    components['Compressor_on']['flag']         =   'ON'
    components['RO_unit']['flag']               =   'ON'
    components['IA_cables']['flag']             =   'ON'
    components[cables]['flag']                  =   'ON'
    components[substr]['flag']                  =   'ON'
    components['H2_onshore']['flag']            =   'ON'
    ...
    for tech in components:
        if components[tech]['flag'] == 'ON':
            if tech == EL:
                component_cost = (components[tech]['specific_CAPEX_'+bound] * components[tech]['ref_size'] *(components[tech]['size']/components[tech]['ref_size'])**SF) * (1 + components[tech]['contingency_costs'] + components[tech]['development_costs']) #[€] electrolyzer cost estimation 
            else: 
            ...
            brine_costs = brine_y * components['RO_unit']['brine_disp_on'] #[€] brine disposal costs
