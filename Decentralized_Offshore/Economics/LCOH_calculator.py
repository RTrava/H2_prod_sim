...
def LCOH_func(sizes, N_EL, components, EL, energy, r, lifetime, SF, bound, rep):
    ...
    components['H2_substation']['size']         =   sizes['substation']         # m^2
    components['H2_local_pipelines']['size']    =   sizes['IA_pipe']            # km
    components['AC_grid_cables']['size']        =   sizes['EX_pipe']
    
    # 'repourposed pipelines'
    if rep: 
        components['H2_On_repur_pipelines']['size']        =   sizes['Backbone_pipe']   # km
        components['H2_off_repur_pipelines']['size']       =   sizes['EX_pipe']         # km
    ...
    # 'new pipelines'
        ...
        components['H2_On_new_pipelines']['size']           =   sizes['Backbone_pipe']      # km
        components['H2_off_new_pipelines']['size']          =   sizes['EX_pipe']            # km
    ...
    components['H2_local_pipelines']['flag']    =   'ON'
    ...
    for tech in components:
        if components[tech]['flag'] == 'ON':
            if tech == EL:
                component_cost = N_EL * components[tech]['specific_CAPEX_'+bound] * components[tech]['ref_size'] *(components[tech]['size']/components[tech]['ref_size'])**SF * (1 + components[tech]['contingency_costs'] + components[tech]['development_costs']) #[€] investment cost initialization
            else: 
            ...
            brine_costs = brine_y * components['RO_unit']['brine_disp_dec'] #[€] brine disposal
        ...
        if n == 0:
            ...
            for tech in components:
                if n%components[tech]['lifetime'] == 0 and components[tech]['flag'] == 'ON':
                    if tech == EL:
                        component_cost = N_EL * components[tech]['specific_CAPEX_'+bound] * components[tech]['ref_size'] *(components[tech]['size']/components[tech]['ref_size'])**SF
                    else: 
